...
(* ------------ ------------ ------------
   ...
   ------------ ------------ ------------
   ------------ ------------ ------------ *)
Definition heap_loc := string.
Definition code_loc := string.
(* Single Units *)
...
(heap * reg_file) -> Prop :=
  | S_add : forall (H:heap) (R:reg_file) (l:location)
    ...
    -> R reg = int_w w1 -> operand_value o R = int_w w2 ->
    step P (H,R)
      (H, ip !->r (next l); reg !->r w1+w2 ; R)
  ...
  | S_salloc : forall (H:heap) (R:reg_file) (l:location)
    ...
    -> R sp =  (incr_loc j lsp) -> H lsp = stack lst -> List.length lst = j ->
    step P (H,R)
      (lsp !->h stack (n_cons n ns lst); H ,
  ...
  | S_push : forall (H:heap) (R:reg_file) (l:location)
    ...
    -> R sp =  (incr_loc j lsp) -> H lsp = stack lst
    -> List.length lst = j ->
    step P (H,R)
  ...
  | S_sfree : forall (H:heap) (R:reg_file) (l:location)
    ...
    -> R reg = incr_loc j lsp -> H lsp = stack lst
  ...
  | S_store : forall (H:heap) (R:reg_file) (l:location)
    ...
    R ip = l -> fetch_instr l P = Some (inl (store d j o))
    -> R d = L -> H L = tuple lst -> j < List.length lst ->
    step P (H,R)
